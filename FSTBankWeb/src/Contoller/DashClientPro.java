...
@WebServlet("/DashClientPro")
public class DashClientPro extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		ArrayList<CProfessionnel> liste = null;
		liste = cpPro.findCompteByClient(idc);
		if (liste.size() != 0) {
			trouve = true;
		} else
			trouve = false;
		...
		this.getServletContext().getRequestDispatcher("/dashClientPro.jsp").forward(request, response);
	}
